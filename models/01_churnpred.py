# ════════════════════════════════════════════════════════
# CHURN PREDICTION — Benchmark 5 Models
...
# ── Pull features from semantic layer ─────────────────────
query = """
SELECT
    c.customer_unique_id,
    COUNT(DISTINCT o.order_id)                          AS frequency,
    ROUND(SUM(p.total_payment)::numeric, 2)             AS monetary,
    CURRENT_DATE - MAX(o.order_purchase_timestamp::timestamp)::date 
                                                        AS recency_days,
...
# ── Create churn label ────────────────────────────────────
# Use dataset's last date as reference, not today
# This avoids everyone being "churned" on a historical dataset
query_max_date = "SELECT MAX(order_purchase_timestamp::timestamp)::date FROM fact_orders"
max_date = pd.read_sql(query_max_date, engine).iloc[0, 0]

df['churned'] = (df['recency_days'] > ML['churn_threshold_days']).astype(int)

# Recalculate recency relative to dataset end date
query2 = f"""
SELECT
    c.customer_unique_id,
    COUNT(DISTINCT o.order_id)                          AS frequency,
...
# ── Prepare features ──────────────────────────────────────
# recency_days removed — it directly defines churn label (data leakage)
# Model must predict churn from behavioural features only
...
# ── Benchmark 5 models ────────────────────────────────────
models = {
    'Logistic Regression' : LogisticRegression(random_state=ML['random_state']),
    'Random Forest'       : RandomForestClassifier(n_estimators=100, random_state=ML['random_state']),
    'Gradient Boosting'   : GradientBoostingClassifier(random_state=ML['random_state']),
    'XGBoost'             : XGBClassifier(random_state=ML['random_state'], eval_metric='logloss'),
}
...
print(f"   Recoverable revenue (15% win-back rate) : R${recoverable_revenue:,.2f}")
